...
# Define a U-Net like model, use pre-trained Faster R-CNN as init
class UNet(nn.Module):
    PIXEL_MEANS = np.array([[[102.9801, 115.9465, 122.7717]]])

    def __init__(self, classes=None, debug=False):
        super(UNet, self).__init__()
        # self.feature = VGG16(bn=False, mid_feat=True)
    ...
    def forward(self, im_data):
        # feat1, feat2, feat3, feat4, feat5 = self.feature(im_data)
    ...
    def predict(self, im_data):
        self.pred = self(im_data)
      
    def load_from_faster_rcnn_h5(self, params):
        pairs = {'rpn.features.conv1.0': 'conv1.0', 'rpn.features.conv1.1': 'conv1.2', 'rpn.features.conv2.0': 'conv2.0', 'rpn.features.conv2.1': 'conv2.2', 'rpn.features.conv3.0': 'conv3.0', 'rpn.features.conv3.1': 'conv3.2', 'rpn.features.conv3.2': 'conv3.4', 'rpn.features.conv4.0': 'conv4.0', 'rpn.features.conv4.1': 'conv4.2', 'rpn.features.conv4.2': 'conv4.4', 'rpn.features.conv5.0': 'conv5.0', 'rpn.features.conv5.1': 'conv5.2', 'rpn.features.conv5.2': 'conv5.4'}
        own_dict = self.state_dict()
        for k, v in pairs.items():
...
class FasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None, rpn_only=False):
